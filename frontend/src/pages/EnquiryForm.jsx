...
function EnquiryForm({ onBackToMenu }) {
  ...
  const handleSubmit = async (e) => {
    e.preventDefault();
    try {
      const response = await axios.post("http://localhost:5000/api/v1/meeting-enquiries", formData);
      setPopup({ show: true, message: response.data?.message || "Meeting enquiry submitted successfully!", type: "success" });
      setFormData({
        name: "", email: "", mobilePhone: "", companyName: "", landPhone: "", companyAddress: "",
        date: "", seating: "", menuType: "", guests: 1, equipment: [], specialRequirement: ""
      });
    } catch (error) {
      setPopup({ show: true, message: error.response?.data?.message || "Failed to send meeting enquiry.", type: "error" });
    }
  };
          ...
          <div className="enquiry-grid">
            ...
            <div className="row-two">
              <div className="input-with-icon">
                <input type="date" name="date" value={formData.date} onChange={handleChange} style={{ width: '100%', padding: '10px' }} required />
              </div>
